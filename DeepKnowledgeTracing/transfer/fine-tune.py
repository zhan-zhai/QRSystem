...
for epoch in tqdm(range(epochs)):
    ...
    logging.info('-' * 10)
    rmse, auc, r2, loss = train_or_evaluate(model, optimizer, train_students, batch_size, num_steps, 124, lr_scheduler)
    print(rmse, auc, r2, loss)
    logging.info(str(rmse) + " " + str(auc) + " " + str(r2)+" "+str(loss))
    # Testing
    if (epoch + 1) % 5 == 0:
        rmse, auc, r2, loss = train_or_evaluate(model, optimizer, test_students, batch_size, num_steps, 124, train=False)
        print('Testing')
        logging.info('Testing')
        print(rmse, auc, r2, loss)
        logging.info(str(rmse) + " " + str(auc) + " " + str(r2) + " " + str(loss))

torch.save(model, 'models/model10.pth')
torch.save(model.state_dict(), 'params/params10.pth')
