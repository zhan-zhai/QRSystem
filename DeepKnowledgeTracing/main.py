...
logging.basicConfig(level=logging.INFO,
                    filename='logs/new14.log',
                    filemode='a',  # 模式，有w和a，w就是写模式，每次都会重新写日志，覆盖之前的日志
                    format='%(asctime)s - %(filename)s[line:%(lineno)d] - %(levelname)s: %(message)s')

parser = argparse.ArgumentParser(description='Deep Knowledge tracing model')
parser.add_argument('-train_data_path', type=str, default='data/0910_a_train.csv', help='Path to the training dataset')
parser.add_argument('-test_data_path', type=str, default='data/0910_a_test.csv', help='Path to the testing dataset')
parser.add_argument('-batch_size', type=int, default=32, help='Batch size for training')
parser.add_argument('-num_layers', type=int, default=1, help='')
parser.add_argument('-hidden_size', type=int, default=250, help='The number of hidden nodes')
...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(args.num_epochs)):
        ...
        logging.info('-' * 10)
        rmse, auc, r2, loss = train_or_evaluate(model, optimizer,  train_students, batch_size, num_steps, num_skills)
        print(rmse, auc, r2, loss)
        logging.info(str(rmse) + " " + str(auc) + " " + str(r2)+" "+str(loss))
        # Testing
        if (epoch + 1) % args.evaluation_interval == 0:
            rmse, auc, r2, loss = train_or_evaluate(model, optimizer, test_students, batch_size, num_steps, num_skills, False)
            print('Testing')
            logging.info('Testing')
            print(rmse, auc, r2, loss)
            logging.info(str(rmse) + " " + str(auc) + " " + str(r2)+" "+str(loss))

    torch.save(model, 'models/model14.pth')
    torch.save(model.state_dict(), 'params/params14.pth')
