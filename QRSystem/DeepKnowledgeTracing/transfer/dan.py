...
src_data_path = '../data/0910_b_train.csv'

logging.basicConfig(level=logging.INFO,
                    filename='logs/new4.log',
...
for epoch in tqdm(range(epochs)):
    ...
    logging.info('-' * 10)
    rmse, auc, r2 = mmd_train(model, optimizer, train_students,src_students, batch_size, num_steps, 124, lr_scheduler, mmd=True)
    print(rmse, auc, r2)
    logging.info(str(rmse) + " " + str(auc) + " " + str(r2))
    # Testing
    if (epoch + 1) % 5 == 0:
        rmse, auc, r2 = mmd_train(model, optimizer, test_students, src_students, batch_size, num_steps, 124, train=False)
        print('Testing')
        logging.info('Testing')
        print(rmse, auc, r2)
        logging.info(str(rmse) + " " + str(auc) + " " + str(r2))
